...
import { getPipelineAssessment, getPipelineSuggestions } from "@/lib/recommendation-engine";
import { getLocalNodeRecommendation, type LocalNodeAlternative } from "@/lib/node-recommendation-engine";
import { localizeAlgorithm, tr, useLocale } from "@/lib/i18n";
...
import { useWorkbenchShortcuts } from "@/features/workbench/hooks/use-workbench-shortcuts";
...
export function useSignalWorkbenchController() {
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      ...
      const firstExecutable = nodesRef.current.find((node) => node.algorithmId !== "analysis.guide");
      if (!canvas || !firstExecutable) return;
      canvas.scrollTo({
        left: Math.max(0, firstExecutable.x * zoomRef.current - 72),
        top: Math.max(0, firstExecutable.y * zoomRef.current - 72),
      });
    });
  ...
  const loadTemplate = (templateId: keyof typeof templates) => {
    ...
    window.setTimeout(() => canvasRef.current?.scrollTo({
      left: Math.max(0, (firstExecutable?.x ?? 0) * zoomRef.current - 72),
      top: Math.max(0, (firstExecutable?.y ?? 0) * zoomRef.current - 72),
      behavior: "smooth",
    }), 20);
  };
  ...
  function flushDragFrame() {
    ...
    for (const nodeId of Object.keys(drag.origins)) {
      const element = stage.querySelector<HTMLElement>(`.graph-node[data-node-id="${CSS.escape(nodeId)}"]`);
      if (element) element.style.transform = `translate3d(${dx}px, ${dy}px, 0)`;
    }

    const draggedIds = new Set(Object.keys(drag.origins));
    const connectedEdges = edges.filter((edge) => draggedIds.has(edge.from) || draggedIds.has(edge.to));
    for (const path of calculateEdgePaths(next, connectedEdges, { nodeWidth: NODE_WIDTH, nodeHeight: NODE_HEIGHT, portOffsetY: 88 })) {
      const group = stage.querySelector<SVGGElement>(`.edge-layer g[data-edge-id="${CSS.escape(path.id)}"]`);
      group?.querySelectorAll<SVGPathElement>("path").forEach((element) => element.setAttribute("d", path.d));
    }
  }
  ...
  function finishDrag() {
    ...
    if (dragRef.current && dragMovedRef.current && dragRef.current.nextNodes) {
      ...
      const finishedNodes = finishedDrag.nextNodes!;
      setTransientNodes(dragRef.current.nextNodes);
      for (const nodeId of Object.keys(finishedDrag.origins)) {
        const element = stageRef.current?.querySelector<HTMLElement>(`.graph-node[data-node-id="${CSS.escape(nodeId)}"]`);
        const node = finishedNodes.find((candidate) => candidate.id === nodeId);
        if (element && node) {
  ...
  useWorkbenchShortcuts({
    ...
    onToggleBypass: toggleSelectedDisabled,
    onEscape: () => {
      setSelectedNodeIds([]);
      setSelectedNodeId(undefined);
      setContextMenu(undefined);
      setInsightNodeId(undefined);
      setLocalRecommendationNodeId(undefined);
    },
  });
  ...
  return {
    locale, setLocale, isMobile, nodes, edges, renderRevision, canUndo, canRedo, dispatchCommand,
